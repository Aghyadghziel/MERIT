...
/**
 * The same, for the Arabic words that stand as posters, in Jomhuria (the
 * Arabic display face, size-adjusted in lib/fonts.ts; Arabic is never tracked): where the first stroke starts
...
export function posterFit(
  ...
  if (!ink) {
    // An unmeasured Arabic word: Jomhuria averages about 0.5em a character.
    const em = arabic
      ? [...text].length * 0.5
