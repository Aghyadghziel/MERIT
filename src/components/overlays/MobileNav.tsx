...
export function MobileNav() {
  ...
  // The second layer slides in from the end side: the right in English, the
  // left in Arabic.
  ...
  useEffect(() => {
    ...
    if (reduced()) {
      gsap.set(el, { x: 0, xPercent: layer ? 0 : off });
    } else if (layer) {
      const tl = gsap.timeline();
      tl.fromTo(el, { x: 0, xPercent: off }, { x: 0, xPercent: 0, duration: 0.7, ease: EASE.cut })
      ...
      const tw = gsap.to(el, { x: 0, xPercent: off, duration: 0.45, ease: 'power3.inOut' });
      return () => { tw.kill(); };
    }
  }, [layer, off]);
  ...
  const item = NAV.find((n) => n.key === layer);
  const saved = ready ? wishlist.length : 0;
  const inBag = ready ? count : 0;

  return (
    <Panel open={isOpen} onClose={close} onClosed={reset} label={t('Menu')} from="full" className="lg:hidden" rootClassName="lg:hidden">
      ...
      <div className="page grid h-(--nav-h) shrink-0 grid-cols-[1fr_auto_1fr] items-center gap-4 border-b border-line md:gap-6">
        <div className="-ms-3 flex items-center justify-self-start">
          <button type="button" className={cn(TAP, 'inline-flex')} onClick={close} aria-label={t('Close menu')}>
            <Burger crossed={isOpen} />
          </button>
          <button type="button" className={cn(TAP, 'inline-flex')} onClick={() => handOff('search')} aria-label={t('Search')}>
            <Icon name="search" />
          </button>
        </div>
        <Link href="/" onClick={close} aria-label={t('MERIT, home')} className="flex h-11 items-center">
          <Wordmark className="h-[22px] w-auto md:h-7" />
        </Link>
        <div className="flex items-center justify-end">
          <Link href="/account" onClick={close} className={cn(TAP, 'hidden sm:inline-flex')} aria-label={t('Account')}>
            <Icon name="account" />
          </Link>
          <Link href="/wishlist" onClick={close} className={cn(TAP, 'inline-flex max-[359px]:hidden')} aria-label={t('Wishlist, {n} saved', { n: saved })}>
            <Icon name="heart" filled={saved > 0} />
          </Link>
          <button
            type="button"
            className="label nums flex min-h-11 shrink-0 items-center gap-2 ps-2"
            onClick={() => handOff('cart')}
            aria-label={t(inBag === 1 ? 'Shopping bag, 1 item' : 'Shopping bag, {n} items', { n: inBag })}
          >
      ...
      <div className="relative flex-1 overflow-hidden">
        ...
        <div className="no-bar flex h-full flex-col overflow-y-auto" inert={Boolean(layer)}>
          <div className="page flex items-center justify-between pb-2 pt-5" data-panel-item>
            <p className="label-sm text-mute">{t('Index')}</p>
            <p className="label-sm nums text-mute">{t('Autumn Winter 2026')}</p>
          </div>

          <nav aria-label={t('Menu')} className="page">
            <ul>
              {NAV.map((n, i) => {
                  ...
                  <>
                    ...
                    <span className="block flex-1 overflow-hidden pb-[0.06em]">
                      <span data-panel-line className="block">
                        {n.label}
                        {active && n.menu ? <span className="sr-only">{t(' (current section)')}</span> : null}
                      </span>
                    </span>
                    {n.menu ? <Icon name="arrowR" className="h-5 w-5 shrink-0 text-mute" /> : null}
                  </>
                );
                const row = 'flex min-h-[3.75rem] w-full items-center gap-3 py-2 text-start text-[clamp(2.25rem,1.1rem+6.2vw,3.75rem)] font-semibold leading-[0.95] tracking-[-0.05em]';
                return (
                  <li key={n.key} className="border-b border-line">
                    {n.menu ? (
                      <button
                        ...
                        data-layer={n.key}
                        className={row}
                        onClick={() => { opener.current = n.key; setLayer(n.key); }}
                        aria-expanded={layer === n.key}
          ...
          <ul className="page mt-6 grid grid-cols-2 gap-x-(--gutter)" data-panel-item>
            <li>
              <Link href="/account" onClick={close} className="label flex min-h-12 items-center gap-3 border-b border-line">
                <Icon name="account" className="h-4 w-4" /> {t('Account')}
              </Link>
            </li>
            <li>
              <Link href="/wishlist" onClick={close} className="label flex min-h-12 items-center gap-3 border-b border-line">
                <Icon name="heart" className="h-4 w-4" filled={saved > 0} /> {t('Wishlist')}
        ...
        <div
          ...
          style={{ transform: `translateX(${off}%)` }}
          aria-hidden={!layer}
          inert={!layer}
        >
          {item?.menu ? (
            <div data-sub-scroll className="no-bar h-full overflow-y-auto pb-10">
              ...
              <div className="page flex items-center justify-between border-b border-line">
                <button
                  ...
                  className="label -ms-1 flex min-h-12 items-center gap-2.5 pe-4"
                >
                  <Icon name="arrowL" className="h-4 w-4" /> {t('Back')}
                </button>
                <Link href={item.href} onClick={close} className="label link-arrow min-h-12 text-mute">
                  {t('All {section}', { section: item.label.toLowerCase() })}
