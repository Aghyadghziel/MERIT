  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const tl = gsap.timeline({
        defaults: { ease: 'none' },
        scrollTrigger: { trigger: '[data-bs="text"]', start: 'top 82%', end: 'bottom 48%', scrub: 0.4 },
      });
      el.querySelectorAll<HTMLElement>('[data-bs="token"]').forEach((t, i) => {
        ...
        if (t.dataset.kind === 'img') {
          ...
          tl.fromTo(pic, { clipPath: 'inset(0% 50% 0% 50%)' }, { clipPath: 'inset(0% 0% 0% 0%)', duration: 0.5, ease: 'power2.out' }, at)
            .fromTo(img, { scale: 1.45 }, { scale: 1, duration: 0.7, ease: 'power2.out' }, at);
        } else {
          tl.fromTo(t, { color: '#cfcbc3' }, { color: '#000000', duration: 0.25 }, at);
        }
      });
    }, el);
    return () => ctx.revert();
            ...
            <Fragment key={i}>
              {'word' in t ? (
                ...
                <span data-bs="token" data-kind="img" aria-hidden title={t.alt}
                  className="relative mx-[0.04em] inline-block h-[0.74em] w-[1.5em] overflow-hidden bg-stone-brand/70 align-[-0.04em]">
